...
public class Vector2D {
    ...
    public void interpolate(Vector2D a, Vector2D b, double scalar) {
        set(b);
        subtract(a);
        multiply(scalar);
        add(a);
    ...
    /**
     * this : Point to project, relative to line's "A" point
     * @param line [AB] segment
     * @return the [AB] ratio from point A where the projected point is
     */
    public double project(Vector2D line) {
        return (this.dot(line) / this.getLength());
    }

    /**
     * this : Point to project
     * @param a,b [AB] segment defined by points A and B
     * @return
     */
    public double orthoLengthSq(Vector2D a, Vector2D b) {
        Vector2D ab = b.getSubtracted(a);
        Vector2D ap = getSubtracted(a);
        double u = ap.dot(ab) / ab.getLengthSq();

        Vector2D pp = ab.getMultiplied(u);
        pp.add(a);

        pp.subtract(this);

        return pp.getLengthSq();
    }
